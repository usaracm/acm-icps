<x-filament-panels::page>
    ...
    <div wire:ignore class="builder-ui keep-selection custom-topbar" style="opacity: 1" data-tooltip>
        <div>
            {{ $this->name ?? '' }} Page
        </div>
    ...
    <script>
        ...
        const builder = new ContentBox({
            ...
            iframeSrc: removeHostFromUrl(
                "{{ WebsiteBuilder\Pages\ContentBuilderBlankPage::getUrl(['doNotShowHeaderFooter' => true, 'layoutShowedName' => $this->name]) }}"
            ),
        ...
        const initialHtml = @json($record['content_html'] ?? '');
        const initialMainCss = @json($record['main_css'] ?? '');
        const initialSectionCss = @json($record['section_css'] ?? '');
        ...
        function preview() {
            ...
            open(@json(route('livewirePageGroup.scheduledConference.pages.home')), '_blank');
        }
